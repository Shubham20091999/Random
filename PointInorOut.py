...
def InterSectionPoint(dot,a,b):#Getting the point of intersection
    add=0.05
    m=1
    while True:
        m+=add#Changing Slope if their is slope problem
...
def CheckInside(dot,pts):
    ...
    if(count%2==0):#if Count is multiple of 2 point is outside
        return False
    return True#if Count is not a multiple of 2 point is outside
